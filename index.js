...
async function start() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const apiKey = 'SLpvUgOcMYwIx0pFeELt' // You can get Apikey at https://mhankbarbars.herokuapp.com/api
			...
			if (type == 'conversation') {
				...
				if (body.startsWith('#tts ')) {
					...
					const tts = require('./lib/tts')(args[1])
					const dtt = body.slice(8)
					if (!dtt) return client.sendMessage(from, 'Masukkan teks.', msgType.text, {quoted: mek})
					if (dtt.length > 600) return client.sendMessage(from, 'Ngotak dong', msgType.text, {quoted: mek})
				...
				} else if (body.startsWith('#ttp ') || body.startsWith('#tstiker ')) {
